...
def evaluate(population, population_size): #Evaluate
    fitness = [] #Initialize Fitness
    for i in range(population_size): #For Population Size
        fitness.append(fitnessFunction(population[i])) #Append Fitness
    return fitness #Return Fitness

def Tournament(population, population_size,tour_size):  #Tournament Selection
...
def singlePointCrossover(parent1, parent2, crossoverProbability):   #Single Point Crossover
...
def mutation(child,mutationProbability, chromosome_length):
